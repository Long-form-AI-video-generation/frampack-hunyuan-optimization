...
from diffusers_helper.wan_components.wan.modules.model import WanAttentionBlock, rope_params,rope_apply,sinusoidal_embedding_1d
import gc
...
class WANDiTBlock(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(
        self,
        in_channels: int = 16,
        ...
        print(f"🚀 Creating WANFramePackTransformer:")
        print(f"   - Inner dim: {self.inner_dim}")
        print(f"   - WAN layers: {num_layers + num_single_layers}")
        print(f"   - Patch size: ({patch_size_t}, {patch_size}, {patch_size})")
        
        # ================================================================
        # FramePack Components (reuse existing)
        # ================================================================
        
        # Input embedders
        self.x_embedder = HunyuanVideoPatchEmbed((patch_size_t, patch_size, patch_size), in_channels, self.inner_dim)
        self.context_embedder = HunyuanVideoTokenRefiner(
            text_embed_dim, num_attention_heads, attention_head_dim, num_layers=num_refiner_layers
        )
        self.time_text_embed = CombinedTimestepGuidanceTextProjEmbeddings(self.inner_dim, pooled_projection_dim)
        
        # Output layers
        self.norm_out = AdaLayerNormContinuous(self.inner_dim, self.inner_dim, elementwise_affine=False, eps=1e-6)
        self.proj_out = nn.Linear(self.inner_dim, patch_size_t * patch_size * patch_size * out_channels)
        
        # ================================================================
        # WAN Components (new architecture)
        # ================================================================
        
        # WAN blocks (replacing dual + single stream)
        total_layers = num_layers + num_single_layers
        self.wan_blocks = nn.ModuleList([
            WanAttentionBlock(
                cross_attn_type='t2v_cross_attn',
                dim=self.inner_dim,
                ffn_dim=self.inner_dim * 4,
        ...
        # WAN-style embeddings
        self.wan_time_embedding = nn.Sequential(
            nn.Linear(256, self.inner_dim),
            nn.SiLU(),
            nn.Linear(self.inner_dim, self.inner_dim)
        )
        self.wan_time_projection = nn.Sequential(
            nn.SiLU(),
            nn.Linear(self.inner_dim, self.inner_dim * 6)
        )
        self.wan_text_embedding = nn.Sequential(
        ...
        # RoPE frequencies
        assert (self.inner_dim % num_attention_heads) == 0 and (self.inner_dim // num_attention_heads) % 2 == 0
        d = self.inner_dim // num_attention_heads
        ...
        # Tensor format conversion layers
        patch_dim = in_channels * patch_size_t * patch_size * patch_size
        self.patch_projection = nn.Linear(patch_dim, self.inner_dim)
        self.patch_unprojection = nn.Linear(self.inner_dim, patch_dim)
        
        print(f"✅ WANFramePackTransformer created successfully!")
    ...
    def forward(
        ...
        **kwargs
        ...
        """Forward pass combining WAN architecture with FramePack logic"""
        
        batch_size, num_channels, num_frames, height, width = hidden_states.shape
        
        print(f"🔄 WANFramePack Forward:")
        print(f"   Input: {hidden_states.shape}")
        
        # ================================================================
        # STEP 1: FramePack-style input processing
        # ================================================================
        
        # Use FramePack's time/text embedding (for compatibility)
        temb = self.time_text_embed(timestep, guidance, pooled_projections)
        encoder_hidden_states = self.context_embedder(encoder_hidden_states, timestep, encoder_attention_mask)
        
        # ================================================================
        # STEP 2: Convert to WAN format
        # ================================================================
        
        # Patchify using FramePack's embedder, then convert to WAN format
        # This maintains compatibility with FramePack's patch embedding
        hidden_states_patches = self.x_embedder.proj(hidden_states)  # (B, C, T, H, W)
        B, C, T, H, W = hidden_states_patches.shape
        
        # Convert to sequence format for WAN: (B, L, D)
        x_wan = hidden_states_patches.flatten(2).transpose(1, 2)  # (B, T*H*W, C)
        
        # ================================================================
        # STEP 3: WAN processing
        # ================================================================
        
        # Move freqs to correct device
        if self.freqs.device != hidden_states.device:
            self.freqs = self.freqs.to(hidden_states.device)
        
        # WAN-style time embeddings
        with torch.cuda.amp.autocast(dtype=torch.float32) if torch.cuda.is_available() else torch.no_grad():
            time_emb = sinusoidal_embedding_1d(256, timestep).float()
            e = self.wan_time_embedding(time_emb)
            e0 = self.wan_time_projection(e).unflatten(1, (6, self.inner_dim))
        
        # WAN-style text embeddings
        context_wan = self.wan_text_embedding(encoder_hidden_states)
        
        # WAN processing parameters
        seq_len = T * H * W
        seq_lens = torch.tensor([seq_len] * batch_size, dtype=torch.long, device=hidden_states.device)
        grid_sizes = torch.tensor([[T, H, W]] * batch_size, dtype=torch.long, device=hidden_states.device)
        
        print(f"   WAN processing: {x_wan.shape}")
        print(f"   Grid sizes: {grid_sizes[0].tolist()}")
        
        # Process through WAN blocks
        for block_idx, block in enumerate(self.wan_blocks):
            x_wan = block(
                x=x_wan,
                e=e0,
                seq_lens=seq_lens,
        ...
        print(f"   WAN complete: {x_wan.shape}")
        
        # ================================================================
        # STEP 4: Convert back and apply FramePack output processing
        # ================================================================
        
        # Convert back to FramePack format
        hidden_states = x_wan.transpose(1, 2).view(B, C, T, H, W)
        
        # Apply FramePack's output normalization and projection
        hidden_states_flat = hidden_states.flatten(2).transpose(1, 2)  # (B, L, C)
        
        # Use FramePack's output layers
        hidden_states_norm = self.norm_out(hidden_states_flat, temb)
        output = self.proj_out(hidden_states_norm)
        
        # Reshape to final output format
        # This needs to match FramePack's expected output format
        output = output.transpose(1, 2).view(B, -1, T, H, W)
        
        # Ensure correct number of output channels
        if output.size(1) != num_channels:
            output = output[:, :num_channels]  # Truncate if necessary
        
        print(f"   Output: {output.shape}")
        print("✅ WANFramePack forward complete!")
    ...
    @classmethod
    def from_hunyuan_pretrained(cls, **kwargs):
        """
        Load from HunyuanVideo pretrained model and convert to WAN architecture
        """
        print(f"🔄 Converting HunyuanVideo model to WAN architecture...")
        
        device = kwargs.pop('device', 'cpu')
        original_kwargs = kwargs.copy()
        kwargs['device_map'] = 'cpu'  
        # Load the original HunyuanVideo model first
        
        original_model = HunyuanVideoTransformer3DModelPacked.from_pretrained(
            'lllyasviel/FramePackI2V_HY', **kwargs
        )
        
        print(f"✅ Original model loaded")
        
        # Create new WAN model with same config
        ...
        num_single_layers=config.get('num_single_layers', 2),
        ...
        print(f"✅ WAN model created")
        
        # Copy compatible weights (embedders, output layers, etc.)
        print(f"🔄 Copying compatible weights...")
        
        # Copy FramePack components that are unchanged
        ...
        for component_name in compatible_components:
            if hasattr(original_model, component_name) and hasattr(wan_model, component_name):
                try:
                    getattr(wan_model, component_name).load_state_dict(
        ...
        print(f"   Original transformer blocks: {len(original_model.transformer_blocks) + len(original_model.single_transformer_blocks)}")
        print(f"   New WAN blocks: {len(wan_model.wan_blocks)}")
        print("🗑️ Freeing original model memory...")
        del original_model
        torch.cuda.empty_cache() if torch.cuda.is_available() else None
        gc.collect()

    
        print(f"✅ Memory freed. WAN model ready.")
        print(f"   Note: WAN blocks start with random weights (will be fine-tuned)")
        
        return wan_model
def test_wan_dit_block():
    """Test WANDiTBlock in isolation"""
    print("Testing WANDiTBlock...")
    
    
    
    model = WANDiTBlock(num_layers=1,          # Small for testing
        num_single_layers=1,   # Small for testing
        num_attention_heads=16,
        attention_head_dim=72)
         
    batch_size = 1
    hidden_states = torch.randn(batch_size, 16, 4, 32, 32)  # Small video
    timestep = torch.tensor([500])
    encoder_hidden_states = torch.randn(batch_size, 77, 4096)  # T5 embeddings
    encoder_attention_mask = torch.ones(batch_size, 77).bool()
    ...
    try:
        ...
        print(f"⚠️ Forward pass failed (expected): {e}")
        print("This is normal - we need to update the forward method next")
        return False
